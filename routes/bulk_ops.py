...
@bp.route('/api/bulk-attributes/preview', methods=['POST'])
def api_bulk_attributes_preview():
    """Preview bulk attribute changes."""
    service = get_service()
    data = request.get_json() or {}
    ...
    action = data.get('action', 'set')  # set, append, prepend, remove

    if not object_type or not target_field:
        return jsonify({'error': 'Object type and target field required'}), 400
    ...
    for idx, obj in enumerate(service.get_objects()):
        ...
        if filter_field and filter_value:
            if filter_field not in obj.attributes:
                continue
            if filter_value.lower() not in obj.attributes[filter_field].lower():
                continue

        old_value = obj.attributes.get(target_field, '')

        # Calculate new value based on action
        if action == 'set':
            calculated_new = new_value
        elif action == 'append':
            calculated_new = old_value + new_value if old_value else new_value
        elif action == 'prepend':
            calculated_new = new_value + old_value if old_value else new_value
        elif action == 'remove':
            calculated_new = ''
        else:
            calculated_new = new_value

        if old_value != calculated_new:
            matches.append({
                'global_index': idx,
                'object': obj.to_dict(),
                'field': target_field,
                'old_value': old_value,
                'new_value': calculated_new
            })
...
@bp.route('/api/bulk-attributes/apply', methods=['POST'])
def api_bulk_attributes_apply():
    """Apply bulk attribute changes."""
    bm = get_backup_manager()
    data = request.get_json() or {}
    ...
    with get_parser_for_modification() as p:
        # Create backup
        backup_path = bm.create_backup("bulk_attribute_edit")
        ...
        for obj in p.objects:
            ...
            if action == 'set':
                obj.attributes[target_field] = new_value
            elif action == 'append':
                obj.attributes[target_field] = old_value + new_value if old_value else new_value
            elif action == 'prepend':
                obj.attributes[target_field] = new_value + old_value if old_value else new_value
            elif action == 'remove':
                if target_field in obj.attributes:
                    del obj.attributes[target_field]

            if obj.attributes.get(target_field, '') != old_value or (action == 'remove' and old_value):
                updated_count += 1
