...
# ---------------------------------------------------------------------------
# Check 24: Services bound to empty hostgroups
# ---------------------------------------------------------------------------

def check_services_on_empty_hostgroups(ctx):
    """Check 24: Services referencing hostgroups with zero members."""
    issues = []
    obj_to_index = ctx["obj_to_index"]
    objects = ctx["objects"]
...
# ---------------------------------------------------------------------------
# Check 25: Redundant escalation contacts
# ---------------------------------------------------------------------------

def check_redundant_escalation_contacts(ctx):
    """Check 25: Escalation contacts identical to base object contacts."""
    issues = []
    obj_to_index = ctx["obj_to_index"]
    template_lookup = ctx["template_lookup"]
    objects = ctx["objects"]
...
# ---------------------------------------------------------------------------
# Check 26: Escalation coverage gaps
# ---------------------------------------------------------------------------

def check_escalation_coverage_gaps(ctx):
    """Check 26: Gaps in escalation notification ranges."""
    issues = []
    obj_to_index = ctx["obj_to_index"]
    objects = ctx["objects"]
...
# ---------------------------------------------------------------------------
# Check 27: Notification period vs criticality mismatch
# ---------------------------------------------------------------------------

def check_notification_period_criticality(ctx):
    """Check 27: Critical services with non-24x7 notification periods."""
    issues = []
    obj_to_index = ctx["obj_to_index"]
    template_lookup = ctx["template_lookup"]
    objects = ctx["objects"]
...
ALL_CHECKS = [
    ...
    check_services_on_empty_hostgroups,  # 24
    check_redundant_escalation_contacts,  # 25
    check_escalation_coverage_gaps,  # 26
    check_notification_period_criticality,  # 27
]
