...
import { getObjectKey, findObjectByKey, getSelectedIndices } from './main.js';
 ...
 */
export function updateReferenceValue(value, oldName, newName) {
...
// Context Menu
export function handleContextMenu(event, index) {
    ...
    const gs = constants.groupStructure;
...
export function contextAction(action) {
    ...
    if (action === 'rename') {
        ...
        const deps = findDependencies(currentName)
        ...
        showDialog('Rename', `
            ...
            <input type="text" id="renameValue" value="${escapeHtml(currentName)}">
            ${refHtml}
        `, applyRename);
        ...
        const fileOptions = allFiles.map(f => {
            ...
            return `<option value="${escapeHtml(f)}"${selected}>${escapeHtml(displayName)}</option>`;
        }).join('');
        showDialog('Clone', `
            <label>New name</label>
...
export function showPreview() {
    ...
    const targetIndex = state.hoveredIndex !== null
        ? state.hoveredIndex
        : Array.from(getSelectedIndices())[0];
...
export function showBulkAction(action) {
    ...
    if (action === 'delete') {
        ...
        const selectedIndices = Array.from(getSelectedIndices());
        const selectedTypes = new Set(
            selectedIndices
...
export function showAddToGroupDialog() {
    hideContextMenu();
        ...
        input.addEventListener('input', () => {
            ...
            if (matches.length === 0) {
                ...
                suggestionsEl.innerHTML = matches.map(g =>
                    `<div class="suggestion-item" data-value="${escapeHtml(g)}">${escapeHtml(g)}</div>`
                ).join('');
            }
...
export function handleDragEnd(event) {
    ...
    updateSelection({ visualOnly: true });
}
...
export async function handleDrop(event, targetFile) {
    event.preventDefault();
    cleanupDragState();
...
window.Explorer.toggleNewFileInput = toggleNewFileInput;
