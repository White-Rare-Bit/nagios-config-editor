...
import { dialogFileSelect, dialogKvList } from './dialogs.js'; // circular — safe (function-level)
import { afterFrontendMutation } from './data-loading.js'; // circular — safe (function-level)
import { getObjectKey } from './main.js'; // circular — safe (function-level)
import { mapHealthCheckToState, renderUnifiedSuggestionsList } from './analysis.js'; // circular — safe (function-level)
 ...
 */
export async function loadTemplateIssues(forceRefresh = false) {
 ...
 */
export function renderTemplateIssues(issues) {
    ...
    if (issues.invalid_use && issues.invalid_use.length > 0) {
        ...
        issues.invalid_use.forEach(issue => {
            ...
            html += `<div class="suggestion-item severity-error" data-action="selectObjectByKey" data-stable-key="${escapeHtml(stableKey)}">`;
            html += `<i class="fa-solid fa-xmark-circle"></i> ${escapeHtml(issue.message)}`;
            html += '</div>';
        });
        html += '</div>';
    }
    ...
    if (issues.unused_templates && issues.unused_templates.length > 0) {
        ...
        issues.unused_templates.forEach(issue => {
            ...
            html += `<div class="suggestion-item severity-info" data-action="selectObjectByKey" data-stable-key="${escapeHtml(stableKey)}">`;
            html += `<i class="fa-solid fa-cube"></i> ${escapeHtml(issue.message)}`;
            html += '</div>';
        });
        html += '</div>';
    }
...
export async function loadTemplateSuggestions(forceRefresh = false) {
        ...
        <div class="template-suggestion" onclick="Explorer.showCreateTemplateDialog(${state.allTemplateSuggestions.indexOf(s)})">
            ...
            <div class="template-suggestion-content">
                <div class="template-suggestion-header">
                    <span class="template-suggestion-name">${escapeHtml(s.suggestedName)}</span>
                    <span class="template-suggestion-type">${escapeHtml(s.type)}</span>
                </div>
            ...
            <div class="template-suggestion-attrs">
                ${Object.keys(s.attributes).slice(0, 5).map(k => `<span class="template-attr">${escapeHtml(k)}</span>`).join('')}
            ...
            <div class="template-suggestion-objects">
                ${s.objects.slice(0, 4).map(o => `<span class="template-object-name">${escapeHtml(getObjectDisplayName(o))}</span>`).join('')}
...
export async function loadGroupingSuggestions(forceRefresh = false) {
...
export function filterGroupingSuggestions() {
    const container = document.getElementById('suggestionsContent');
    ...
    container.innerHTML = filtered.map((s, idx) => `
        <div class="suggestion-item" onclick="Explorer.showCreateGroupDialog(${state.allGroupingSuggestions.indexOf(s)})">
            ...
            <div class="suggestion-members">
                ${s.members.slice(0, 5).map(m => `<span class="suggestion-member">${escapeHtml(m)}</span>`).join('')}
...
export function showCreateTemplateDialog(idx) {
    ...
    showDialog('Create Template', `
        <label>Template Name</label>
        <input type="text" id="newTemplateName" value="${escapeHtml(suggestion.suggestedName)}">
        ${dialogFileSelect('newTemplateFile', 'Target File', defaultFile)}
        ...
        <div class="dialog-checkbox-group">
            ...
            <div class="dialog-scrollable-list dialog-scrollable-list--short code-preview">
                ${suggestion.objects.map(o => `<span class="template-object-name">${escapeHtml(getObjectDisplayName(o))}</span>`).join(' ')}
            </div>
        </div>
    `, async () => {
...
export function showCreateGroupDialog(idx) {
    ...
    showDialog('Create Hostgroup', `
        ...
        <input type="text" id="newGroupName" value="${escapeHtml(suggestion.name)}">
        ${dialogFileSelect('newGroupFile', 'Target File', defaultFile)}
        <label>Members (${suggestion.members.length} hosts)</label>
        <div class="dialog-scrollable-list">
            ${suggestion.members.map(m => `<span class="suggestion-member">${escapeHtml(m)}</span>`).join(' ')}
        </div>
    `, async () => {
...
// ==========================================================================
// Window exports for onclick HTML strings
// ==========================================================================

window.Explorer.showCreateTemplateDialog = showCreateTemplateDialog;
window.Explorer.showCreateGroupDialog = showCreateGroupDialog;
