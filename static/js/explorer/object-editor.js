...
import { getObjectKey, findObjectByKey, parseCommaValues } from './main.js';
import { afterFrontendMutation, getStagingHeaders } from './data-loading.js'; // circular — safe (function-level)
import { buildLocalInheritanceChain } from './relations-loader.js'; // circular — safe (function-level)
import { isSelectedByIndex } from './state-management.js';
import { openTab, renderTabBar } from './tab-manager.js'; // circular — safe (function-level)
import { getIcon, handleApiError, toRelativePath, toDisplayPath, extractFileName } from './ui-utils.js';
 ...
 */
export function validateRequiredFields(objectType, attributes) {
    const errors = [];
...
function formatIssueBadge(issue, obj, displayName) {
    ...
    let issueLabel = getIssueShortLabel(issue);
...
export function updateIssueBadge(issueBtn, issue, obj, hostListInfo) {
...
export function syncCenterPaneAfterUndo() {
    ...
    const obj = findObjectByKey(objKey);
    if (!obj) {
        hideCenterPaneObject();
        return;
    }
...
export function showCenterPaneMultiple(count) {
    const emptyState = document.getElementById('centerEmptyState');
    const content = document.getElementById('centerContent');
...
export function getAttributeSuggestions(attrName, objectType) {
 ...
 */
export function syncHighlight(textarea) {
...
function showDocsPopover(targetEl, directive, objectType) {
    ...
    el.innerHTML =
        '<div class="attr-docs-header">' +
            '<code class="attr-docs-name">' + escapeHtml(directive.name) + '</code>' +
            '<span class="attr-docs-badge ' + badgeClass + '">' + badgeText + '</span>' +
        '</div>' +
        '<div class="attr-docs-format">Format: <code>' + escapeHtml(directive.format) + '</code></div>' +
        '<div class="attr-docs-desc">' + escapeHtml(directive.description) + '</div>' +
 ...
 */
export function hideDocsPopover() {
...
export function filterCommaValueSuggestions(inputValue, allSuggestions, attrKey) {
...
export function showAttrAutocomplete(input, attrKey) {
...
export function showAutocompleteDropdown(input, suggestions, attrKey, options = {}) {
...
export function hideAttrAutocomplete(event) {
    hideAutocompleteDropdown('.attr-autocomplete', () => {
        state.currentAutocompleteKey = null;
        state.highlightedIndex = -1;
    });
}

export function selectAttrAutocomplete(attrKey, value) {
...
export function handleAttrAutocompleteKey(event, attrKey) {
    handleAutocompleteKeyNav(event, {
...
export function copyAttributeValue(key) {
...
export function showAddAttribute() {
    ...
    `, () => {
        ...
        if (name) {
            ...
            refreshRelatedSections(name, state.editedObject);
...
export function showAddAttrNameAutocomplete() {
...
export function hideAddAttrNameAutocomplete() {
    hideAutocompleteDropdown('#addAttrNameDropdown', () => {
        state.addAttrNameHighlightedIndex = -1;
    });
}

export function selectAddAttrNameAutocomplete(value) {
    const input = document.getElementById('newAttrName');
    if (input) {
        input.value = value;
    }
...
export function handleAddAttrNameAutocompleteKey(event) {
    handleAutocompleteKeyNav(event, {
...
export function showAddAttrAutocomplete() {
...
export function hideAddAttrAutocomplete() {
    hideAutocompleteDropdown('#addAttrDropdown', () => {
        state.addAttrHighlightedIndex = -1;
    });
}

export function selectAddAttrAutocomplete(value) {
    const input = document.getElementById('newAttrValue');
...
export function handleAddAttrAutocompleteKey(event) {
    handleAutocompleteKeyNav(event, {
...
export function saveDetailSectionState() {
    try {
        const sectionState = {};
...
export function restoreDetailSectionState() {
    try {
 ...
 */
export async function fetchInheritance(stableKey) {
    // Check cache first
 ...
 */
export function invalidateInheritanceCache(stableKey) {
    if (stableKey) {
 ...
 */
export function buildStableKey(obj) {
    const objName = obj.display_name ?? getEffectiveName(obj);
 ...
 */
export async function loadInheritanceSection(obj) {
...
export function renderInheritanceSection(data, obj) {
...
// onclick/oninput handlers in generated HTML — must be on window.Explorer
window.Explorer = window.Explorer || {};
window.Explorer.showAttrAutocomplete = showAttrAutocomplete;
window.Explorer.hideAttrAutocomplete = hideAttrAutocomplete;
window.Explorer.handleAttrAutocompleteKey = handleAttrAutocompleteKey;
window.Explorer.updateAttribute = updateAttribute;
window.Explorer.syncHighlight = syncHighlight;
window.Explorer.copyAttributeValue = copyAttributeValue;
window.Explorer.deleteAttribute = deleteAttribute;
window.Explorer.selectAttrAutocomplete = selectAttrAutocomplete;
window.Explorer.showAddAttrNameAutocomplete = showAddAttrNameAutocomplete;
window.Explorer.hideAddAttrNameAutocomplete = hideAddAttrNameAutocomplete;
window.Explorer.handleAddAttrNameAutocompleteKey = handleAddAttrNameAutocompleteKey;
window.Explorer.selectAddAttrNameAutocomplete = selectAddAttrNameAutocomplete;
window.Explorer.showAddAttrAutocomplete = showAddAttrAutocomplete;
window.Explorer.hideAddAttrAutocomplete = hideAddAttrAutocomplete;
window.Explorer.handleAddAttrAutocompleteKey = handleAddAttrAutocompleteKey;
window.Explorer.selectAddAttrAutocomplete = selectAddAttrAutocomplete;
