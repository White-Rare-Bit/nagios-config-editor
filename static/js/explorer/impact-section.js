 ...
 */
export async function loadImpactAndRelationships(obj) {
    ...
    if (!state.isNewObject && getObjectKey(obj)) {
        try {
            const result = await ApiClient.get(`/api/object-references?key=${encodeURIComponent(getObjectKey(obj))}`);
...
async function gatherInheritanceData(obj) {
    ...
    if (useAttr) {
        const templateNames = parseCommaValues(useAttr);
    ...
    if (!state.isNewObject && useAttr) {
        try {
            ...
            const inheritData = await fetchInheritance(stableKey);
...
function renderAncestryChain(chain, objectType) {
    ...
    flat.forEach((node, idx) => {
        ...
        const clickHandler = isMissing ? '' : `onclick="Explorer.selectObjectByName('${escapeJs(node.name)}')"`;

        const badgeLabel = node.is_template ? `${objectType} template` : objectType;
        html += `<span class="${itemClass}" ${clickHandler} title="${escapeHtml(displayName)}">`;
...
function renderParentHostsChain(tree) {
    ...
    flat.forEach((node, idx) => {
        ...
        html += `<span class="${itemClass}" ${clickHandler} title="${escapeHtml(node.name)}">`;
...
function renderIncomingSubsection(obj, incoming) {
    ...
    const objName = escapeHtml(getEffectiveName(obj));
    const nameField = (constants.nameFields || {})[obj.object_type] || obj.object_type;
    ...
    sorted.forEach(ref => {
        ...
        content += `<span class="impact-diff-name" title="${escapeHtml(displayName)}">${escapeHtml(displayName)}</span>`;
        content += `<span class="impact-diff-reason">${escapeHtml(reason)}</span>`;
        content += '</div>';
    });
    content += '</div>';
...
function renderOutgoingSubsection(outgoing) {
    ...
    outgoing.forEach(ref => {
        ...
        content += `<span class="dep-grid-name" title="${escapeHtml(displayName)}">${escapeHtml(displayName)}</span>`;
        content += `<span class="dep-grid-attr">${escapeHtml(attrKey)}</span>`;
        content += '</div>';
    });
    content += '</div>';
...
function renderMembershipSubsection(obj, members, memberOf, transitiveSummary) {
    ...
    if (memberOf.length > 0) {
        ...
        memberOf.forEach(item => {
            ...
            content += `<span class="dep-grid-status">[✓] <span class="dep-grid-type">${type}</span></span>`;
            content += `<span class="dep-grid-name" title="${escapeHtml(displayName)}">${escapeHtml(displayName)}</span>`;
            content += `<span class="dep-grid-attr">${escapeHtml(via)}</span>`;
            content += '</div>';
        });
        content += '</div>';
    }
    ...
    if (members.length > 0) {
        ...
        if (transitiveSummary && transitiveSummary.transitive_count > 0) {
            ...
            `;
            if (intermediateList) {
                content += `<div class="dep-rule-details"><div class="dep-rule-detail"><i class="fa-solid fa-layer-group"></i> Via: ${escapeHtml(intermediateList)}</div></div>`;
            }
        }
...
function renderResolvedAttrsTable(resolvedAttrs, obj) {
    ...
    for (const [key, rawValue] of entries) {
        ...
        html += `<span class="resolved-attr-name">${escapeHtml(key)}</span>`;
        html += `<span class="resolved-attr-value" title="${escapeHtml(value)}">${escapeHtml(value)}</span>`;
        html += `<span class="resolved-attr-source ${sourceClass}">${escapeHtml(source)}</span>`;
        html += '</div>';
    }
...
// =============================================================================
// BACKWARD COMPAT — expose on Explorer namespace for inline onclick handlers
// =============================================================================

window.Explorer = window.Explorer || {};
window.Explorer.loadImpactAndRelationships = loadImpactAndRelationships;
window.Explorer.navigateToObjectByIndex = navigateToObjectByIndex;
window.Explorer.selectObjectByName = selectObjectByName;
