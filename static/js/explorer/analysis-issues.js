...
import { dialogInfoText, dialogFileSelect, dialogKvList } from './dialogs.js';
...
export async function loadIssues() {
    ...
    if (!result.success) {
        if (container) {
            container.innerHTML = `<div class="tab-placeholder">Error: ${escapeHtml(result.error)}</div>`;
        }
        return;
    }
...
export function buildGroupedErrors(combined) {
    ...
    for (const groupType of groupOrder) {
        ...
        for (const { group, idx } of items) {
            if (group.objectType) {
                ...
                html += `
                    <div class="cleanup-suggestion cleanup-error" data-index="${idx}" onclick="Explorer.showGroupedErrorDetail(${idx})">
                        <div class="cleanup-info">
                            <div class="cleanup-title">${escapeHtml(title)}</div>
                            <div class="cleanup-desc">${escapeHtml(desc)}</div>
                        </div>
                ...
                html += `
                    <div class="cleanup-suggestion cleanup-error" data-index="${idx}" onclick="Explorer.navigateToIssue('${escapeJs(issue.object)}', '${escapeJs(issue.object_type)}')">
                        <div class="cleanup-info">
                            <div class="cleanup-title">${escapeHtml(title)}</div>
                            <div class="cleanup-desc">${escapeHtml(issue.message)}</div>
                        </div>
                    </div>
                `;
            }
...
export function showGroupedErrorDetail(idx) {
    ...
    const objectList = group.issues.map(issue => {
        return `<div class="ref-item ref-item-clickable" onclick="Explorer.navigateToIssue('${escapeJs(issue.object)}', '${escapeJs(issue.object_type)}'); Explorer.closeDialog();">
            <span class="ref-type-badge type-${issue.object_type}">${escapeHtml(issue.object_type)}</span>
            <span class="ref-name" title="${escapeHtml(issue.object)}">${escapeHtml(issue.object)}</span>
        </div>`;
    }).join('');

    showDialog(`Missing ${group.objectType}: ${group.missingName}`, `
        ${dialogInfoText(`This ${group.objectType} is referenced by ${group.issues.length} object(s) but doesn't exist.`)}
        <p class="u-mb-sm"><strong>Affected objects:</strong></p>
        <div class="dialog-scrollable-list">${objectList}</div>
    `, null);
}

export function resolveGroupedError(idx) {
    const group = state.groupedErrors[idx];
...
export function getIssueResolveInfo(issue) {
 ...
 */
export function createAllMissing(groupType) {
    ...
    const itemsList = matchingGroups.map(g => {
        ...
        return `<div class="batch-create-item">
            <span class="batch-create-name">${escapeHtml(g.missingName)}</span>
    ...
    showDialog(`Create All Missing ${typeLabel.charAt(0).toUpperCase() + typeLabel.slice(1)}`, `
        ${dialogInfoText(`Create ${matchingGroups.length} missing ${typeLabel} at once.`)}
        ${dialogFileSelect('batchCreateTargetFile', 'Target File', defaultFile)}
        <div class="u-mb-md">
 ...
 */
export function buildDefaultAttributes(objectType, objectName, isTemplate) {
    const nameField = isTemplate ? 'name' : (constants.nameFields[objectType] || 'name');
    const attributes = {};
...
export function openCreateObjectForIssue(objectType, objectName, targetFile, issue) {
    ...
    const nameField = isTemplate ? 'name' : (constants.nameFields[effectiveType] || 'name');
...
export function showCreateObjectForIssueDialog(objectType, attributes, suggestedFile, isTemplate) {
    ...
    showDialog(`Create Missing ${typeLabel}`, `
        ${dialogInfoText(`Create a new ${typeLabel} to resolve this issue.`)}
        ${dialogFileSelect('resolveTargetFile', 'Target File', suggestedFile)}
        <div class="u-mb-md">
            <label class="form-label">Attributes</label>
            <div class="code-preview">
                ${dialogKvList(kvPairs)}
            </div>
...
export function navigateToIssue(objectName, objectType) {
...
// ==========================================================================
// Window exports for onclick handlers in HTML strings
// ==========================================================================

window.Explorer.createAllMissing = createAllMissing;
window.Explorer.showGroupedErrorDetail = showGroupedErrorDetail;
window.Explorer.resolveGroupedError = resolveGroupedError;
window.Explorer.navigateToIssue = navigateToIssue;
