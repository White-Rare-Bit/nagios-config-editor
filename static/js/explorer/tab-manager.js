...
import { findObjectByKey, getObjectKey } from './main.js';
import { checkForChanges, showCenterPaneObject, hideCenterPaneObject } from './object-editor.js'; // circular — safe (function-level)
import { clearSelection, isSelectedByIndex } from './state-management.js';
import { updateSelection, getObjectTypeIcon } from './app.js'; // circular — safe (function-level)
...
function restoreTabs() {
    try {
        ...
        for (const tab of openTabs) {
            const obj = findObjectByKey(tab.key);
            if (obj) {
                state.openTabs.push({
                    key: tab.key,
...
function closeTab(key) {
    ...
    if (state.activeTabKey === key) {
        if (state.openTabs.length === 0) {
            ...
            const obj = findObjectByKey(newTab.key);
            if (obj) {
                refreshBadgesForActiveTab();
                showCenterPaneObject(obj);
                syncTreeSelection(obj);
...
function syncTreeSelection(obj) {
    ...
    sessionStorage.setItem('explorerSelectedKey', getObjectKey(obj));
...
export function renderTabBar() {
    ...
    const tabsHtml = state.openTabs.map(tab => {
        ...
        const icon = getObjectTypeIcon(tab.typeIcon);
        const escapedKey = escapeHtml(tab.key);
...
export { openTab, closeTab, activateTab, restoreTabs, validateTabs, persistTabs, syncTreeSelection };
