...
/**
 * Populate constants from /api/metadata response.
 ...
 */
export function getTypeBadge(objectType, isTemplate) {
    const c = constants;
 ...
 */
export function getTypeBadgeTier(objectType, isTemplate, tier) {
    const c = constants;
 ...
 * Shared implementation — do not duplicate in other modules.
 */
export function stripPrefix(val) {
 ...
 */
export function checkDuplicateName(objectType, name, attributes, excludeStagedIndex) {
    if (!name) {return {isDuplicate: false, location: ''};}

    const c = constants;
    const nameField = c.nameFields[objectType] || 'name';
    const isHostScoped = constants.hostScopedTypes.has(objectType);
