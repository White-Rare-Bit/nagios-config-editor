...
(function() {
    ...
    async function loadObjects() {
        ...
        // Show loading state
        container.innerHTML = '<div class="text-muted text-center p-3">Loading...</div>';
        ...
        if (result.success) {
            ...
            container.innerHTML = '<div class="text-muted text-center p-3">Failed to load objects.</div>';
    ...
    function displayObjectList(objects) {
        ...
        const html = objects.map(obj => `
            <div class="object-list-item" data-name="${escapeHtml(obj.name)}" onclick="InheritanceViewer.selectObject('${escapeJs(obj.name)}')">
                <div>
    ...
    async function selectObject(name) {
        ...
        // Update UI to show selection using data-name attribute
        document.querySelectorAll('.object-list-item').forEach(item => {
            item.classList.toggle('active', item.dataset.name === name);
        });

        // Show loading state
        document.getElementById('chainContainer').innerHTML = '<div class="text-muted text-center p-4">Loading...</div>';
        document.getElementById('resolvedContainer').innerHTML = '<div class="text-muted text-center p-4">Loading...</div>';
        ...
        if (!result.success) {
            ...
            document.getElementById('chainContainer').innerHTML = '<div class="text-muted text-center p-4">Failed to load inheritance chain.</div>';
            document.getElementById('resolvedContainer').innerHTML = '';
            return;
        }
    ...
    function displayResolvedAttributes(resolved, selfName) {
        ...
        const html = `
            <div class="resolved-attrs-scroll">
                <div class="resolved-attr resolved-attrs-header">
