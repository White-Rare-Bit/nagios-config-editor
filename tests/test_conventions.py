...
class TestPythonNaming:
            ...
            try:
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.FunctionDef):
                        if camel_pattern.match(node.name):
                            violations.append(f"{py_file.name}:{node.lineno} - {node.name}")
            except SyntaxError:
                continue

        assert not violations, f"Found camelCase function names:\n" + "\n".join(violations)
    ...
    def test_method_names_snake_case(self):
                        ...
                        for item in node.body:
                            if isinstance(item, ast.FunctionDef):
                                ...
                                if not item.name.startswith('_'):
                                    if camel_pattern.match(item.name):
                                        violations.append(
        ...
        assert not violations, f"Found camelCase method names:\n" + "\n".join(violations)
...
class TestJavaScriptNaming:
    ...
    def test_no_snake_case_functions(self):
        ...
        assert not violations, f"Found snake_case function names:\n" + "\n".join(violations)
    ...
    def test_no_snake_case_variables(self):
        ...
        assert not violations, f"Found snake_case variable names:\n" + "\n".join(violations)
...
class TestCSSNaming:
    ...
    def test_class_names_kebab_case(self):
        ...
        assert not violations, f"Found non-kebab-case class names:\n" + "\n".join(violations)
    ...
    def test_css_variables_namespaced(self):
        ...
        assert not violations, f"Found non-namespaced CSS variables:\n" + "\n".join(violations)
...
class TestDesignTokens:
    ...
    def test_no_hardcoded_colors_outside_tokens(self):
        ...
        if violations:
            pytest.xfail(f"Found hardcoded colors (consider using tokens):\n" + "\n".join(violations[:10]))
...
class TestTemplateInheritance:
    ...
    def test_templates_extend_base(self):
        ...
        assert not violations, f"Templates not extending base.html:\n" + "\n".join(violations)
...
class TestEventDelegation:
    ...
    def test_no_inline_onclick(self):
        ...
        assert not violations, f"Found inline onclick handlers:\n" + "\n".join(violations)
...
class TestApiClientUsage:
    ...
    def test_no_direct_fetch_for_api(self):
        ...
        assert not violations, f"Found direct fetch calls to /api:\n" + "\n".join(violations)
...
class TestThreadSafety:
    ...
    def test_uses_multiprocessing_lock(self):
        ...
        assert not violations, f"Files using threading.Lock instead of multiprocessing.Lock:\n" + "\n".join(violations)
...
class TestServiceAccess:
    ...
    def test_routes_use_helper_functions(self):
        ...
        assert not violations, f"Found direct current_app.extensions access:\n" + "\n".join(violations)
...
class TestOperationResultPattern:
    ...
    def test_no_dict_returns_in_service(self):
        ...
        assert not violations, f"Found dict returns that should be OperationResult"
